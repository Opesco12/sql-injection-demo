...
const modeText = {
  vulnerable: {
    ...
    explanation:
      "Vulnerable Mode: Input is inserted directly into the SQL string. Attackers can manipulate query logic.",
  },
  protected: {
    label: "Protected Mode",
    explanation:
      "Protected Mode: Parameterized queries treat input as data. User values cannot alter SQL structure.",
  },
};
...
function updateModeUI() {
  ...
  modeExplanation.innerHTML = `<strong>${escapeHtml(modeText[currentMode].label)}:</strong> ${escapeHtml(modeText[currentMode].explanation.split(": ")[1])}`;
...
function renderLoginOutcome(payload) {
  ...
  resultMessage.classList.add(payload.success ? "success" : "error");
}
